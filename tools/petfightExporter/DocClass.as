package {
    ...
    public class DocClass extends MovieClip {
        ...
        private static const RealFrames:Array = [1, 4, 6];
        private static const RealFaces:Array = [1, 2];
        private var _curFaceIndex:int;
        private var _curFrameIndex:int;
...
//            }
        ...
        private function onLoadComplete(event:Event):void {
            ...
            var raceId:int = parseInt(fileFullName.split(".")[0].substr("pet".length));
            trace("当前亚比id" + raceId);
            _curFrameIndex = _curFaceIndex = 0;
            this.convertThisPet(raceId);
        }

        private function convertThisPet(raceId:int):void {
            _realCurFace = RealFaces[_curFaceIndex];
            _realCurFrame = RealFrames[_curFrameIndex];
            var sourceMovie:MovieClip = this.getSourceMovie(raceId);
            if (sourceMovie == null) {
                this.afterConvert(raceId);
                return;
            }
            sourceMovie.gotoAndStop(_realCurFrame);
            if(sourceMovie.numChildren == 0){
                this.afterConvert(raceId);
            }else{
                ...
                new Transfer(this.movieContainer,this.curProcessFile(),sourceMovie,raceId,this._exportDir, this._realCurFrame,this._realCurFace, this.afterConvert).doTransfer();
            }
        }

        private function afterConvert(raceId:int):void {
            _curFrameIndex++;
            if (_curFrameIndex >= RealFrames.length) {
                _curFaceIndex++;
                _curFrameIndex = 0;
            }
            if (_curFaceIndex >= RealFaces.length) {
                convertNext();
            }
            else {
                convertThisPet(raceId);
            }
        }

        private function getSourceMovie(raceId:int):MovieClip{
            var clsName:String = "mmo.petfight.pet"+raceId+"_"+_realCurFace;
            var sourceMovieCls:Class;
            try {
                sourceMovieCls = getDefinitionByName(clsName) as Class;
            }
            catch (e:Error){
                return null;
            }
