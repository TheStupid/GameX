const FILE_NAME_PREFIX = "type";
const LINKAGE_CLASS_NAME_PREFIX = "mmo.petcard.CardView";
const ICON_WIDTH = 33;
...
function getKey(fileName){
	return fileName.substr(FILE_NAME_PREFIX.length);
}
...
function exportIcon(doc,item,key){
	...
	item.x = 0;
	item.y = 0;
	doc.exportPNG(exportFolderURL + "/export/type" + key + ".png",true,true);
}
