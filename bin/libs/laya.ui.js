...
var Button=(function(_super){
	...
	*/
	__proto.onMouse=function(e){
		...
		//>>Mod ligenhao
		//加上enabled判断
		this.enabled && !this._selected && (this.state=Button.stateMap[e.type]);
		//<<
	}
...
var ComboBox=(function(_super){
	...
	*/
	__getset(0,__proto,'isOpen',function(){
		...
		if (this._isOpen !=value && this._button){
			this._isOpen=value;
...
var ScrollBar=(function(_super){
	...
	__proto.initialize=function(){
		...
		this.upButton.stateNum = 3;
		this.downButton.stateNum = 3;
	...
	*/
	...
	__proto.onStageMouseUp2=function(e){
		...
		if(this._target)this._target.mouseEnabled=true;
		if (this._isElastic){
			if (this._value < this.min){
	...
	*/
	__getset(0,__proto,'max',function(){
		return this.slider ? this.slider.max : 0;
		},function(value){
		if(this.slider)this.slider.max=value;
	});
	...
	*/
	__getset(0,__proto,'min',function(){
		return this.slider ? this.slider.min : 0;
		},function(value){
		if(this.slider)this.slider.min=value;
	});
	...
	*/
	__getset(0,__proto,'isVertical',function(){
		return this.slider ? this.slider.isVertical : false;
		},function(value){
		if(this.slider)this.slider.isVertical=value;
	});
	...
	*/
	__getset(0,__proto,'sizeGrid',function(){
		return this.slider ? this.slider.sizeGrid : "";
		},function(value){
		if(this.slider)this.slider.sizeGrid=value;
	});
	...
	*/
	__proto.initItems=function(){
		...
		for (var i = 0; i < this.numChildren; i++) {
            var item = this.getChildAt(i);
            var matcher = item.name.match(/(item|frame)([0-9]+)/);
            if (matcher != null) {
            	var curIndex = parseInt(matcher[2]);
            	if (matcher[1] == "frame") {
                    curIndex --;
                    item.name = "item" + curIndex;
				}
